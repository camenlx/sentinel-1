...
sys.path.append(os.path.normpath(os.path.join(os.path.dirname(__file__), 'bin')))
import sentinel
sys.path.append(os.path.normpath(os.path.join(os.path.dirname(__file__), 'lib')))
import config
import absolute_config
...
def fix_masternode(data_folder):
    try:
        ...
        if not os.path.isfile(wallet_file):
            print('It seems like the data folder (the one containing wallet.dat) is not the same as the folder where wallet conf is')
            data_folder = input('Please, write the data folder path: ')
            return fix_masternode(data_folder)

        print(colored('Make a copy of "wallet.dat" and "absolute.conf" before continuing.\nThis program will try it best not to touch them, but just in case!', attrs=['bold']))
        ...
        for filename in os.listdir(data_folder):
            if filename in ('wallet.dat', 'absolute.conf'):
                continue
        ...
        print(colored('Checking absolute.conf contents', 'green'))
        tokens = absolute_config.AbsoluteConfig.tokenize(config.absolute_conf)
        ...
        try:
            ...
            r = None
            while r not in ('y', 'n'):
                print(colored('It seems like your absolute.conf is not setting the wallet as masternode!', 'red'))
        ...
        if 'masternodeprivkey' not in tokens:
            print(colored('Your absolute.conf does not contain a \'masternodeprivkey\' entry, please set it up before opening your wallet', 'red'))

        # Rewrite config
        with open(config.absolute_conf, 'w') as fp:
            for k, v in tokens.items():
                fp.write('{}={}\n'.format(k, v))

        print(colored('Done checking\n', 'green'))
        print(colored('Please open your wallet (absoluted on linux) and wait for it completely load/open before continuing', attrs=['bold']))
        ...
        print(colored('If it says it can not connect, check your absolute.conf settings', 'yellow'))
...
if __name__ == '__main__':
    ...
    print(colored('Using absolute.conf: {}'.format(config.absolute_conf), 'green'))
    ...
    if option == 1: 
        ...
        data_folder = os.path.dirname(config.absolute_conf)
