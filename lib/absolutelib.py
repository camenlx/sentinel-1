...
def is_valid_absolute_address(address, network='mainnet'):
    ...
    absolute_version = 120 if network == 'testnet' else 55
...
# shims 'til we can fix the absoluted side
def SHIM_serialise_for_absoluted(sentinel_hex):
    ...
    # superblock => "trigger" in absoluted
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # absoluted expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    absoluted_hex = serialise(obj)
    return absoluted_hex


# shims 'til we can fix the absoluted side
def SHIM_deserialise_from_absoluted(absoluted_hex):
    from models import DASHD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(absoluted_hex)
